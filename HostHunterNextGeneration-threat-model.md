...
| AP-21 | Leak credentials or leave changed endpoint policy after qualification | Native qualification → passphrase in process configuration, lingering agent identity, wrong cleanup order, or wrong Keychain service cleanup → reusable authority or changed audit policy | access/integrity | low | high | medium | Interactive passphrases only, run-scoped agent, exact identity removal and agent stop, policy restoration through the retained password profile after remote-key removal, production-derived Keychain services, pre-delete presence and post-delete absence checks | qualification scripts and focused contract tests; exact native receipts pending |
...
| AP-15 | Swap, truncate, or misparse output evidence | Move valid ciphertext between rows/invocations, remove final chunks, or induce locale-dependent parsing → misleading or unavailable audit output | integrity | low | high | medium | Database/ledger/invocation AAD, chained chunks/footer, DB hash/length, culture-independent raw timestamp parsing, durable no-replace publication and retrieval verification | artifact/query implementation and tests |
