...
| R6 | Fast container flow | Use one production-derived verifier and one disposable SSH target | `verify-cmdlets.sh` emits a 12-row receipt with no retry | verified: 12/12 passed; SQLite integrity `ok` | no unit/coverage/scans/build in cmdlet verdict |
...
| R13 | Radical release coverage simplification | Remove custom AST instrumentation and use one standard native unit-only profiler over all shipped source | statements, executable lines, and invoked functions >=90%; behavioral branch tests; complete failure receipt | implemented; future exact-SHA result pending | no integration/live numerator, synthetic branch gate, or production exclusions |
...
| R12 | Remove dirty-tree blockage | Preserve the experiment externally, then restore clean `31d6baa` | verified patch checksum, five-file untracked inventory, and clean baseline | verified | experimental AST work is reference-only and is not reapplied |
...
| Mission and host details | bounded producer, schema validation, partial-field projection, stable opaque identity | authenticated schema-v3 persistence and 12-row journey | working-tree journey verified; exact-SHA release proof pending |
...
The retained development verifier has passed all twelve rows in one invocation.
Its receipt is a working-tree artifact bound to the module and image digests;
it is not clean exact-SHA release evidence.
