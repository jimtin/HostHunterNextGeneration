...
| R5 | Preserve security guarantees | Retain authenticated DB, keys, anchors, encrypted output and recovery | focused tamper/rollback/recovery integration | implemented | release-only heavy proof has not run for uncommitted work |
...
| R11 | Heavy proof cannot hide cmdlet result | Store cmdlet receipt independently; run coverage/security/build once in release aggregation | read-only aggregator preserves both verdicts | verified | no automatic reruns |
