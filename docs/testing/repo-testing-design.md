...
The package exports exactly seventeen PowerShell cmdlets. The canonical controller
...
`FunctionsToExport` in the packaged module manifest is the sole executable
authority for the framework-command set. Package validation, controller startup,
native-client qualification, and cmdlet receipts all compare against that list.
The unit contract contains the only intentional numeric assertion that the
current product exports seventeen commands.
...
disposable SSH fixture. One ordered stateful journey calls all seventeen framework cmdlets,
...
The same verifier invocation performs a fail-closed preflight before the first
cmdlet. It proves manifest/journey agreement, the committed migration inventory,
fresh writable state, fixture username/fingerprint/password readability, and
receipt-directory write plus `fsync`. The password is opened only to prove that
it is readable and non-empty; its content is never copied into a receipt, log,
argument, environment value, or client frame. Fixture-secret group `10002` and
the host artifact group are independent supplementary groups.

Development attempts are written to
`.artifacts/cmdlets/<source-fingerprint>/<run-id>/cmdlets/receipt.json`, so a
dirty-tree edit cannot overwrite or masquerade as an earlier attempt. Every
terminal path records the phase and a bounded, redacted error. Release proof
continues to use the immutable `.artifacts/cmdlets/<sha>` identity supplied by
the exact-SHA gate.
...
3. run the seventeen-cmdlet verifier once;
...
cannot alter or hide the seventeen-row cmdlet or Windows verdict.
...
seventeen-cmdlet journey never contribute to the coverage numerator. Statements,
...
static/governance checks, one ordinary unit smoke, and one seventeen-cmdlet/SQLite
