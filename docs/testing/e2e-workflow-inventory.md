...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Import module and inspect help | module package | local operator | clean package | exactly eight cmdlets and resolvable help without loading SQLite or creating state | `HH_TEST_MODULE_PATH` clean packaged import | manifest/export/help/lazy-loader contract | verified working tree |
...
| Verify SQLite-only persistence | all eight cmdlets | local operator | complete journey set | no active flow creates or reads target/audit JSON or JSONL | filesystem sweep and legacy negatives | deleted-surface/reference guard | verified working tree |
