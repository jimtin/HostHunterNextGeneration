...
| Windows controller qualification | Provider/RID/ACL/reparse plus PS7/5.1/mixed/bootstrap proof | Native/live release | Available Windows x64 laptop | `pwsh -File scripts/qualification/Test-HHWindowsController.ps1 -CandidateSha <sha> -PackageArchivePath <package> -SshHost <host> -UserName <user> -HostKeyFingerprint <fingerprint>` | repo-owned | Every exact release candidate | implemented; candidate execution pending | Same package SHA-256 as exact-SHA gate; credentials remain interactive |
