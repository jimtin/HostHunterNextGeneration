...
Describe 'SSH Ed25519 key bootstrap' -Tag Unit {
    ...
    It 'adds both password and key-only runtime identity operations for Windows PowerShell 5.1' {
        $prepared = Prepare-HHSshKeyBootstrapOperation `
            -Target (New-HHBootstrapTestTarget -PowerShellRuntime WindowsPowerShell51) `
            -KnownHostsPath $script:knownHostsPath `
            -KeyPath $script:keyPath `
            -KeyGenerator { param($path) Write-HHBootstrapTestKeyPair -Path $path } `
            -Confirm:$false
        ...
        @($prepared.RemoteOperations.PowerShellRuntime) | Should -Be @(
            'PowerShell7',
            'WindowsPowerShell51',
            'PowerShell7',
            'PowerShell7',
    ...
    It 'accepts only complete self-consistent RuntimeMismatch evidence projections' {
        $newMismatch = {
            param(
                [object] $Identity = (New-HHBootstrapIdentity -PowerShellRuntime WindowsPowerShell51),
                [string] $Version = '5.1.26100.9168',
            ...
            $failure = New-HHSshClassifiedException `
                ...
                -Message 'synthetic observed mismatch'
            $failure.Data['HHObservedIdentity'] = $Identity
            $failure.Data['HHObservedRemotePowerShellVersion'] = $Version
            $failure.Data['HHObservedRemotePSEdition'] = $Edition
            $failure.Data['HHObservedExecutionMode'] = $ExecutionMode
            $failure.Data['HHObservedValidatedAtUtc'] = $ValidatedAtUtc
            $failure.Data['HHObservedHostKeyFingerprint'] = $script:hostFingerprint
            $failure
        }

        (Get-HHSshBootstrapObservedMismatchContext `
                -ErrorObject ([TimeoutException]::new('not a mismatch')) `
                -RequestedPowerShellRuntime PowerShell7 `
                -ExpectedHostKeyFingerprint $script:hostFingerprint) |
            Should -BeNullOrEmpty

        $incompleteIdentity = (New-HHBootstrapIdentity -PowerShellRuntime WindowsPowerShell51).
            PSObject.Copy()
        $incompleteIdentity.UserName = ''
        (Get-HHSshBootstrapObservedMismatchContext `
                -ErrorObject (& $newMismatch -Identity $incompleteIdentity) `
                -RequestedPowerShellRuntime PowerShell7 `
                -ExpectedHostKeyFingerprint $script:hostFingerprint) |
            Should -BeNullOrEmpty

        (Get-HHSshBootstrapObservedMismatchContext `
                -ErrorObject (& $newMismatch -Edition Core) `
                -RequestedPowerShellRuntime PowerShell7 `
                -ExpectedHostKeyFingerprint $script:hostFingerprint) |
            Should -BeNullOrEmpty
        (Get-HHSshBootstrapObservedMismatchContext `
                -ErrorObject (& $newMismatch -Version invalid) `
                -RequestedPowerShellRuntime PowerShell7 `
                -ExpectedHostKeyFingerprint $script:hostFingerprint) |
            Should -BeNullOrEmpty
        (Get-HHSshBootstrapObservedMismatchContext `
                -ErrorObject (& $newMismatch -ValidatedAtUtc 'not-a-time') `
                -RequestedPowerShellRuntime PowerShell7 `
                -ExpectedHostKeyFingerprint $script:hostFingerprint) |
            Should -BeNullOrEmpty

        $wrongProcess = (New-HHBootstrapIdentity -PowerShellRuntime WindowsPowerShell51).
            PSObject.Copy()
        $wrongProcess.ProcessPath = '/opt/microsoft/powershell/7/pwsh'
        (Get-HHSshBootstrapObservedMismatchContext `
                -ErrorObject (& $newMismatch -Identity $wrongProcess) `
                -RequestedPowerShellRuntime PowerShell7 `
                -ExpectedHostKeyFingerprint $script:hostFingerprint) |
            Should -BeNullOrEmpty

        $matchingIdentity = New-HHBootstrapIdentity
        (Get-HHSshBootstrapObservedMismatchContext `
                -ErrorObject (& $newMismatch `
        ...
        $windowsRequestedMismatch = Get-HHSshBootstrapObservedMismatchContext `
            -ErrorObject (& $newMismatch `
                -Identity $matchingIdentity `
                -Version '7.6.5' `
                -Edition Core `
                -ExecutionMode WindowsPowerShellCompatibility) `
            -RequestedPowerShellRuntime WindowsPowerShell51 `
            -ExpectedHostKeyFingerprint $script:hostFingerprint
        $windowsRequestedMismatch.RemotePSEdition | Should -BeExactly 'Core'
    }
    ...
    It 'preserves complete password-session RuntimeMismatch evidence before installation' {
        Write-HHBootstrapTestKeyPair -Path $script:keyPath
        ...
        $result = Invoke-HHSshKeyBootstrap `
            ...
            -RemoteInvoker {
                ...
                if ($remoteScript.ToString().Contains('HostHunter.PowerShellIdentity.v1')) {
                    New-HHBootstrapIdentity -PowerShellRuntime WindowsPowerShell51
                }
                else {
                    $script:installCalls++
                }
        ...
        $result.RemotePowerShellVersion | Should -BeExactly '5.1.26100.9168'
        ...
        $result.RemoteIdentity.ProcessPath | Should -Match 'powershell\.exe$'
        ...
        {
            Assert-HHTransportAuditResult `
                -TransportResult $result `
                -IntentMetadata ([pscustomobject]@{
    ...
    It 'preserves key-only RuntimeMismatch evidence after proving exact compensation' {
        Write-HHBootstrapTestKeyPair -Path $script:keyPath
        ...
        $remoteInvoker = {
            ...
            if ($text.Contains('HostHunter.PowerShellIdentity.v1')) {
                if ($session.Authentication -ceq 'PublicKey') {
                    New-HHBootstrapIdentity -PowerShellRuntime WindowsPowerShell51
                }
                else {
    ...
    It 'accepts a key-only RuntimeMismatch when installation made no mutation' {
        Write-HHBootstrapTestKeyPair -Path $script:keyPath
        $script:rollbackCalls = 0
        $remoteInvoker = {
            param($session, $remoteScript, $argumentList)
    ...
    It 'returns generic Unknown state when a key-only mismatch cannot be compensated' {
        Write-HHBootstrapTestKeyPair -Path $script:keyPath
        $remoteInvoker = {
            param($session, $remoteScript, $argumentList)
            $null = $argumentList
            ...
            elseif ($text.Contains('HostHunterAuthorizedKeyRollback.v1')) {
                throw 'rollback state unavailable'
            }
        }
        $result = Invoke-HHSshKeyBootstrap `
            -Target $script:passwordTarget `
    ...
    It 'preserves an explicit Windows PowerShell 5.1 runtime through key conversion' {
        $windowsPowerShellTarget = New-HHBootstrapTestTarget `
        ...
        $remoteInvoker = {
            ...
            elseif ($text.Contains('HostHunterAuthorizedKeyInstall.v1')) {
                ...
                throw 'Unexpected outer-runtime bootstrap command.'
            }
        }
        $bridgeInvoker = {
            New-HHBootstrapEnvelope `
                -Kind Stream `
                -Sequence 0 `
                -Value (New-HHBootstrapIdentity -PowerShellRuntime WindowsPowerShell51)
            New-HHBootstrapEnvelope -Kind Completion -Sequence 1 -Value $null
        }

        $result = Invoke-HHSshKeyBootstrap `
            -Target $windowsPowerShellTarget `
            -KnownHostsPath $script:knownHostsPath `
            -KeyPath $script:keyPath `
            -KeyGenerator { param($path) Write-HHBootstrapTestKeyPair -Path $path } `
            -SessionFactory $sessionFactory `
        ...
        $result.ProfileTransition.PowerShellRuntime | Should -BeExactly 'WindowsPowerShell51'
        $result.ProfileTransition.LastValidatedPSEdition | Should -BeExactly 'Desktop'
        $result.ProfileTransition.LastValidatedPowerShellVersion | Should -BeExactly '5.1.26100.9168'
        $result.ProfileTransition.LastValidatedExecutionMode |
            Should -BeExactly 'WindowsPowerShellCompatibility'
        @($script:sessionPlans.Target.PowerShellRuntime) |
            Should -Be @('WindowsPowerShell51', 'WindowsPowerShell51')
    }
