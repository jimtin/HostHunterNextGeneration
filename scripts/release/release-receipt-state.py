...
def record(args: argparse.Namespace) -> int:
    ...
    if args.source:
        ...
        allowed = {
            ...
            "scope", "rowCount", "rows", "controllerImageId",
        }
...
def seal_value(root: pathlib.Path, sha: str, status: str, phase: str, exit_code: int,
    ...
    cmdlet_path = root / "cmdlet-receipt.json"
    heavy_path = root / "heavy-receipt.json"
    windows_path = root / "windows-receipt.json"
    cmdlet = load_json(cmdlet_path) if cmdlet_path.exists() else None
    heavy = load_json(heavy_path) if heavy_path.exists() else None
    windows = load_json(windows_path) if windows_path.exists() else None
    return {
        "schemaVersion": 1,
        "state": "terminal",
        "attempt": 1,
        ...
        "exitCode": exit_code,
        "reason": reason,
        "cmdletVerdict": verdict_summary(cmdlet),
        "heavyProofVerdict": verdict_summary(heavy),
        "windowsQualificationVerdict": verdict_summary(windows),
    }
...
def aggregate(args: argparse.Namespace) -> int:
    ...
    value = {
        ...
        "cmdletVerdict": verdict_summary(load_json(cmdlet_path) if cmdlet_path.exists() else None),
        "heavyProofVerdict": verdict_summary(load_json(heavy_path) if heavy_path.exists() else None),
        "windowsQualificationVerdict": verdict_summary(
            load_json(windows_path) if windows_path.exists() else None
        ),
        "terminalReceipt": "receipt.json",
        "receiptSha256": {
            "terminal": sha256_file(root / "receipt.json"),
            "cmdlet": sha256_file(cmdlet_path) if cmdlet_path.exists() else None,
            "heavyProof": sha256_file(heavy_path) if heavy_path.exists() else None,
            "windowsQualification": sha256_file(windows_path) if windows_path.exists() else None,
        },
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    item.add_argument("--root", required=True)
    item.add_argument("--sha", required=True)
    item.add_argument("--kind", required=True, choices=("cmdlet", "heavy", "windows"))
